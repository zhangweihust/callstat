...
public class ErrorReporter implements Thread.UncaughtExceptionHandler {
	...
	void handleException(Throwable e) {
		try {
			...
			ILog.logException(this.getClass(), ex);
		}
	}

	private void showCrashDialog() {
		// Log.i("i", "in showDialog");
	...
	/*
	 * private static void saveLogToFile(final String file) { try{ final Process
	 * process = Runtime.getRuntime().exec( String.format(LOG_COMMAND_TEMP,
	 * file)); process.waitFor(); final int exitCode = process.exitValue();
	 * Log.i(LOG_TAG, "saveLogToFile finished.exitCode = " +
	 * exitCode);//$NON-NLS-1$ } catch (final Exception e){ Log.e(LOG_TAG,
	 * "saveLogToFile failed", e);//$NON-NLS-1$ } }
	 */
}