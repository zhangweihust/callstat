				...
				if (c != null && c.getCount() > 0) {
					for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
						...
						if (type == Calls.OUTGOING_TYPE) {
							...
							numberType = com.archermind.callstat.monitor.bean.CallLog.CALL_FREE;
						}
						numType = numberType;
						...
						if (c.isLast()) {
							...
							if (tm.isNetworkRoaming()) {
								numberType = com.archermind.callstat.monitor.bean.CallLog.CALL_ROAMING;
							}
						}
						com.archermind.callstat.monitor.bean.CallLog call = new com.archermind.callstat.monitor.bean.CallLog(
								number, date, type, duration, numberType);

						int monthly_stat_data_src_type = MonthlyStatDataSource.UNKNOWN;

						if (numberType == com.archermind.callstat.monitor.bean.CallLog.CALL_LOCAL) {
							monthly_stat_data_src_type = MonthlyStatDataSource.LOCAL;
						} else if (numberType == com.archermind.callstat.monitor.bean.CallLog.CALL_LONG_DISTANCE) {
							monthly_stat_data_src_type = MonthlyStatDataSource.LONG;
						} else if (numberType == com.archermind.callstat.monitor.bean.CallLog.CALL_ROAMING) {
							monthly_stat_data_src_type = MonthlyStatDataSource.ROAMING;
						} else if (numberType == com.archermind.callstat.monitor.bean.CallLog.CALL_SHORT) {
							monthly_stat_data_src_type = MonthlyStatDataSource.SHORT;
						} else if (numberType == com.archermind.callstat.monitor.bean.CallLog.CALL_IP) {
							monthly_stat_data_src_type = MonthlyStatDataSource.IP;
						} else if (numberType == com.archermind.callstat.monitor.bean.CallLog.CALL_UNKONW) {
						...
						if (db != null) {
							...
							db.createTable(CallStatDatabase.TABLE_USER_CONSUME_MONTHLY_STATISTIC);// create
																									...
																									// not
																									// exists
							if (numberType != com.archermind.callstat.monitor.bean.CallLog.CALL_FREE) {
